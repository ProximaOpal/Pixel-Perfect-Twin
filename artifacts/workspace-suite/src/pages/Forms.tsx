...
/* ─── preview images for dropdown hover ─── */
import img1 from '@assets/image_1783807219263.png';
import img2 from '@assets/image_1783807510122.png';
import img3 from '@assets/image_1783805723793.png';
import img4 from '@assets/Your_Business_at_a_Glance_1783778728657.jpg';
import img5 from '@assets/Process_by_Eddie_Lobanovskiy_1783778873710.jpg';

const VESSEL_TYPES = ['WEOTT I', 'Avon Tour', 'London Rose', 'Dixie'];
const EVENT_TYPES = ['Corporate Summer', 'Networking', 'Christmas', 'Wedding'];
...
/* ─── Custom Select with hover preview trigger ─── */
function FormSelect({
  label,
  field,
  options,
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute left-0 right-0 top-full z-20 border border-[#e0e0e0] bg-white shadow-lg"
          >
            {options.map((opt) => (
              <motion.li
                key={opt}
...
/* ─── Dummy lead for overlay context ─── */
function makeLead(data: FormData, field: string): Lead {
  const vessel = data.vesselType || 'Vessel';
  const event = data.eventType || 'Event';
  return {
    id: 1,
    name: `${vessel} — ${event}`,
    email: 'events@nexus.co.uk',
    code: 'NX-2026',
    designation: 'Proposal in progress',
    phone: '+44 20 7946 0958',
    joined: new Date().toLocaleDateString('en-GB'),
    color: '#2ecc71',
    initials: 'NX',
    sector: data.menuType || 'Hospitality',
    referenceNumber: `REF-${Date.now().toString().slice(-6)}`,
    source: 'Proposal Wizard',
    company: 'Nexus Events Ltd',
    linkedin: undefined,
  };
}
...
export function Forms() {
  ...
  const [overlayLead, setOverlayLead] = useState<Lead | null>(null);
  ...
  const handleSelect = (field: string, _value: string) => {
    setOverlayLead(makeLead(data, field));
  };

  const fin = calcFinancials(data);
  const previewImg = hoverField ? PREVIEW_MAP[hoverField] : null;
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] bg-white">
      {/* ── Top step bar ── */}
      <div className="sticky top-16 z-30 flex w-full border-b border-[#e8e8e8] bg-white">
        {STEP_LABELS.map((label, i) => {
          ...
          return (
            <button
              key={label}
              onClick={() => setStep(n)}
              className={`flex flex-1 items-center justify-center gap-2 py-4 text-[11px] font-bold uppercase tracking-widest transition-colors ${
                active
            ...
            >
              <span
                className={`flex h-5 w-5 items-center justify-center text-[10px] font-black ${
      ...
      {/* ── Right-edge hover image preview ── */}
      <AnimatePresence>
      ...
      {/* ── Centered overlay ── */}
      <LeadPanel lead={overlayLead} onClose={() => setOverlayLead(null)} />
    </div>
  );
}
